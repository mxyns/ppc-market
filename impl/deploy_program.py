...
def deploy_program(homes_queue_key, market_queue_key, max_market_turns, to_kill, silent):
    ...
    market_args = dict(
        ...
        shared_time=shared_time,
        economics_events=[
            make_event(name="shortage", prob=0.01, sig=signal.SIGPIPE, lifespan=10, handler=toggle_external_factor,
                       args=("shortage", 3))
        ],
        politics_events=[
            make_event(name="war", prob=0.005, sig=signal.SIGUSR1, lifespan=20, handler=toggle_external_factor,
                       args=("war", 1.3)),
            make_event(name="tensions", prob=0.08, sig=signal.SIGUSR2, lifespan=8, handler=toggle_external_factor,
                       args=("tensions", 1.2))
        ],
