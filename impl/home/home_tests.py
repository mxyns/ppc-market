...
from impl.home.home_comm_utils import get_message
...
def queue_tests():
    ...
    print("1")
    a = get_message(queue=Q, type_id=0, block=True)
    print(a)
    Q.send("mdr", type=1)
    print("2")
    a = get_message(queue=Q, type_id=1, block=False)
    print(a)
    Q.send("ptdr", type=1)
    print("3")
    a = get_message(queue=Q, type_id=0, block=False)
    print(a)
    Q.send("xptdr", type=1)
    Q.send("xptdr 2 ", type=1)
    Q.send("lol", type=2)
    print("4")
    a = get_message(queue=Q, type_id=1, block=True)
    print(a)
    a = get_message(queue=Q, type_id=2, block=False)
    print(a)
    print("should block")
    a = get_message(queue=Q, type_id=2, block=False)
    print("passed ?!")
