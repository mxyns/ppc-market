...
# TODO some kind of shit like this for market-home communication multiplexing
...
def retrieveEnergyTransfers(owner, queue):
    ...
    message, id = getMessage(queue=queue, type_id=energy_transfer_id(owner.id), block=False)
    print(f"{owner.id} => read ({message, id})")
    while message is not None:
        messages.append((message, id))
        print(f"{owner.id} => yes the bois sent me {float(message)} energy yeehaaww")
        message, id = getMessage(queue=queue, type_id=energy_transfer_id(owner.id), block=False)
    print(f"{owner.id} => no more transfers for me :(")
    return messages
...
def acceptEnergyTransfersIfAny(owner, queue):
    ...
    print(f"house {owner.id} accepted {len(replies)} transfers")


def cancelRequest(owner, queue):
    # empties the message queues from all of the owner's requests
    getLastMessage(queue=queue, type_id=energy_request_id(owner.id))
    print(f"house {owner.id} cancelled its energy request")
